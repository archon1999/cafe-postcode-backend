...
from apps.orders.models import Order, OrderItem, Payment, PaymentRefund, Receipt
from apps.orders.services import CashShiftService, OrderPaymentService, OrderStateService, PaymentRefundService
...
class OrderPaymentServiceTests(PosTestCase):
    ...
    def _create_order_with_item(self, *, channel, quantity=1, table_session=None):
        order = Order.objects.create(
            ...
            distribution_point=self.hall_distribution if channel == Order.Channel.HALL else self.takeaway_distribution,
            opened_by=self.user,
            order_number=1,
            channel=channel,
    ...
    def test_process_takeaway_payment_keeps_service_fee_zero(self):
        order = self._create_order_with_item(channel=Order.Channel.TAKEAWAY, quantity=2)

        result = self.service.process(
            order=order,
            payload={'method': Payment.Method.CASH, 'amount': 60000},
            received_by=self.user,
            cash_shift=self.create_cash_shift(),
        )
...
class PaymentRefundServiceTests(PosTestCase):
    ...
    def test_refund_creates_refund_record_and_receipt(self):
        ...
        self.assertEqual(refund_result['refund'].amount, 30000)
    ...
    def test_reprint_increments_receipt_counter(self):
        ...
        result = self.payment_service.process(
            order=order,
            payload={'method': Payment.Method.CARD, 'amount': 30000},
            received_by=self.user,
            cash_shift=shift,
        )
