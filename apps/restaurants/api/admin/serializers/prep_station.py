...
class PrepStationSerializer(serializers.ModelSerializer):
    ...
    def to_internal_value(self, data):
        if isinstance(data, dict):
            ...
            if isinstance(cook_ids, list):
                ...
                mutable_data["cook_ids"] = [
                    str(cook_id) for cook_id in cook_ids if str(cook_id) in allowed_ids
                ]
    ...
    def validate(self, attrs):
        ...
        if printer is not None and (
            ...
            or printer.kind != IntegrationConfig.Kind.PRINTER
        ):
            raise serializers.ValidationError(
                {"printer_integration": "Selected printer integration is invalid."}
            )
        return attrs
