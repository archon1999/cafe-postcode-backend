...
class CashDeskSerializer(serializers.ModelSerializer):
    ...
    def validate_enabled_payment_methods(self, value):
        ...
        if "mixed" in values and not {"cash", "card"}.issubset(set(values)):
            raise serializers.ValidationError(
                _("Mixed payment requires cash and card payment methods.")
            )
        return values
    ...
    def validate_fiscal_integration(self, value):
        ...
        if restaurant is not None and value.restaurant_id != restaurant.id:
            raise serializers.ValidationError(
                _("Selected fiscal integration belongs to another restaurant.")
            )
        if value.kind != IntegrationConfig.Kind.FISCAL:
            raise serializers.ValidationError(
                _("Selected integration is not a fiscal integration.")
            )
        if not value.is_enabled:
            raise serializers.ValidationError(
                _("Selected fiscal integration is disabled.")
            )
        return value
    ...
    def validate_payment_integration(self, value):
        ...
        if restaurant is not None and value.restaurant_id != restaurant.id:
            raise serializers.ValidationError(
                _("Selected payment integration belongs to another restaurant.")
            )
        if value.kind != IntegrationConfig.Kind.PAYMENT:
            raise serializers.ValidationError(
                _("Selected integration is not a payment integration.")
            )
        if value.provider != "marta-softpos":
            raise serializers.ValidationError(
                _("Selected payment integration is not a MARTA SoftPOS integration.")
            )
        if not value.is_enabled:
            raise serializers.ValidationError(
                _("Selected payment integration is disabled.")
            )
        return value
    ...
    def validate_printer_integration(self, value):
        ...
        if restaurant is not None and value.restaurant_id != restaurant.id:
            raise serializers.ValidationError(
                _("Selected printer integration belongs to another restaurant.")
            )
        if value.kind != IntegrationConfig.Kind.PRINTER:
            raise serializers.ValidationError(
                _("Selected integration is not a printer integration.")
            )
        if not value.is_enabled:
            raise serializers.ValidationError(
                _("Selected printer integration is disabled.")
            )
        return value
    ...
    def get_fiscal_integration_name(self, obj):
        ...
        return (
            f"{integration.provider} ({terminal_id})"
            if terminal_id
    ...
    def get_payment_integration_name(self, obj):
        ...
        return (
            f"{integration.provider} ({endpoint_url})"
            if endpoint_url
            else integration.provider
        )

    def get_printer_integration_name(self, obj):
        return get_printer_integration_display(
            getattr(obj, "printer_integration", None)
        )
