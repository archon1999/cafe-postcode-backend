...
from apps.catalog.models import CatalogCategory, ModifierGroup, ModifierOption
...
class AdminScopeHeaderTests(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.permission = Permission.objects.get_or_create(
            ...
            defaults={
                ...
                "description": "Catalog category create permission",
            },
        )[0]
    ...
    def test_superuser_admin_post_requires_restaurant_header(self):
        ...
        response = self.client.post(
            "/api/v1/admin/catalog/categories/", self.create_payload(), format="json"
        )
    ...
    def test_superuser_admin_post_accepts_valid_restaurant_header(self):
        ...
        self.assertTrue(
            CatalogCategory.objects.filter(
                pk=response.data["id"], restaurant=self.restaurant
            ).exists()
        )
    ...
    def test_superuser_list_without_header_includes_restaurant_names(self):
        ...
        response = self.client.get("/api/v1/admin/catalog/categories/")
    ...
    def test_superuser_modifier_list_without_header_includes_all_branches(self):
        first_group = ModifierGroup.objects.create(
            restaurant=self.restaurant, name="First extras"
        )
        second_group = ModifierGroup.objects.create(
            restaurant=self.second_restaurant, name="Second extras"
        )
        ModifierOption.objects.create(group=first_group, name="First option")
        ModifierOption.objects.create(group=second_group, name="Second option")
        self.client.force_authenticate(self.superuser)

        response = self.client.get("/api/v1/admin/catalog/modifier-groups/")
