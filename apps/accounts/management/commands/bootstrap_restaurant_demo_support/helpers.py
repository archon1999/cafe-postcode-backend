...
def configure_feature_config(restaurant: Restaurant, spec, tariff: Tariff) -> FeatureConfig:
    feature_config, _ = FeatureConfig.objects.get_or_create(restaurant=restaurant)
    feature_config.hall_enabled = spec.hall_enabled
    feature_config.kitchen_enabled = spec.kitchen_enabled
    feature_config.cashier_enabled = spec.cashier_enabled
    feature_config.owner_dashboard_enabled = spec.owner_dashboard_enabled
    feature_config.order_entry_mode = spec.order_entry_mode
    feature_config.kitchen_mode = spec.kitchen_mode
    feature_config.enabled_modules = MODULES_BY_TARIFF[spec.tariff_key]
    feature_config.enabled_roles = list(tariff.allowed_roles.values_list('code', flat=True))
    feature_config.entitlement_overrides = {}
    feature_config.save()
