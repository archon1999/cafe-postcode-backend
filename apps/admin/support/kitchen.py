...
KITCHEN_TICKET_STATUS_VALUES = {choice for choice, _label in KitchenTicket.Status.choices}
KITCHEN_TICKET_ROUTE_MODE_VALUES = {choice for choice, _label in KitchenTicket.RouteMode.choices}
...
def admin_kitchen_ticket_queryset(request) -> QuerySet[KitchenTicket]:
    return (
...
class KitchenTicketListFilters:
    ...
    def from_request(cls, request) -> 'KitchenTicketListFilters':
        ...
        prep_station_ids = list(get_str_list_query_param(query_params, 'prep_station_id_in'))
        ...
        statuses = list(get_str_list_query_param(query_params, 'status_in', allowed_values=KITCHEN_TICKET_STATUS_VALUES))
        ...
        routed_via_values = list(
            get_str_list_query_param(query_params, 'routed_via_in', allowed_values=KITCHEN_TICKET_ROUTE_MODE_VALUES)
        )
        ...
        return cls(
            ...
            ordering=get_ordering_query_param(query_params, KITCHEN_TICKET_ORDERING_FIELDS),
        )

    def apply(self, queryset: QuerySet[KitchenTicket]) -> QuerySet[KitchenTicket]:
        if self.search:
            search_query = (
...
class AdminKitchenTicketQuerysetMixin:
    def get_kitchen_ticket_queryset(self) -> QuerySet[KitchenTicket]:
        return admin_kitchen_ticket_queryset(self.request)

    def get_filtered_kitchen_ticket_queryset(self) -> QuerySet[KitchenTicket]:
        return KitchenTicketListFilters.from_request(self.request).apply(self.get_kitchen_ticket_queryset())
