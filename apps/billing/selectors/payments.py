...
def admin_payment_queryset(request) -> QuerySet:
    return (
        filter_queryset_by_optional_scope(
            Payment.objects.all(), request, restaurant_lookup="order__restaurant"
        )
        .select_related(
            "order", "order__restaurant", "cash_desk", "cash_shift", "received_by"
        )
...
def admin_receipt_queryset(request) -> QuerySet:
    return (
        filter_queryset_by_optional_scope(
            Receipt.objects.all(), request, restaurant_lookup="order__restaurant"
        )
        .select_related("order", "order__restaurant", "payment")
        .order_by("-created_at")
    )
...
class PaymentListFilters:
    ...
    def from_request(cls, request) -> "PaymentListFilters":
        ...
        return cls(
            ...
            statuses=tuple(
                get_str_list_query_param(
                    query_params, "status_in", allowed_values=PAYMENT_STATUS_VALUES
                )
            ),
            methods=tuple(
                get_str_list_query_param(
                    query_params, "method_in", allowed_values=PAYMENT_METHOD_VALUES
                )
            ),
...
class ReceiptListFilters:
    ...
    def from_request(cls, request) -> "ReceiptListFilters":
        ...
        return cls(
            ...
            statuses=tuple(
                get_str_list_query_param(
                    query_params, "status_in", allowed_values=RECEIPT_STATUS_VALUES
                )
            ),
            kinds=tuple(
                get_str_list_query_param(
                    query_params, "kind_in", allowed_values=RECEIPT_KIND_VALUES
                )
            ),
