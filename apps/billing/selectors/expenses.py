...
def admin_expense_category_queryset(request) -> QuerySet:
    return filter_queryset_by_optional_scope(ExpenseCategory.objects.all(), request).order_by('sort_order', 'name')
...
class CashExpenseListFilters:
    ...
    def from_request(cls, request):
        ...
        return cls(
            ...
            statuses=tuple(get_str_list_query_param(params, 'status_in', allowed_values={'posted', 'voided'})),
            category_ids=tuple(get_str_list_query_param(params, 'category_id_in', aliases=('category_id',))),
