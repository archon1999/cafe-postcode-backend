    ...
    def validate_name(self, value):
        ...
        if request is not None:
            ...
            queryset = ExpenseCategory.objects.filter(
                restaurant=restaurant, name__iexact=value
            )
...
class AdminCashExpenseSerializer(serializers.ModelSerializer):
    ...
    category_name = serializers.CharField(
        source="category_name_snapshot", read_only=True
    )
    recipient_name = serializers.CharField(
        source="recipient_name_snapshot", read_only=True
    )
    created_by_name = serializers.CharField(
        source="created_by.full_name", read_only=True
    )
    voided_by_name = serializers.CharField(
        source="voided_by.full_name", read_only=True, allow_null=True
    )
