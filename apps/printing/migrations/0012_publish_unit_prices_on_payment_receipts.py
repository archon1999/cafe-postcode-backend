...
def replace_payment_line_totals(layout):
    changed = False
    for block in layout.get('blocks', []):
        ...
        for column in columns:
            if column.get('value') == '{{item.lineTotal}}':
                column['value'] = '{{item.unitPrice}}'
                changed = True
    return changed


def publish_payment_unit_prices(apps, schema_editor):
    ...
    for template in templates.iterator():
        ...
        if not replace_payment_line_totals(layout):
            continue
